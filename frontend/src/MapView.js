...
import oceanLabels from "./ocean_labels"; // <-- our ocean names
...
export default function MapView() {
  ...
  useEffect(() => {
    fetch(
      (process.env.REACT_APP_BACKEND || "http://localhost:8000") + "/floats"
    )
      .then((r) => r.json())
      .then(setFloats)
      .catch(() => setFloats([]));
  ...
  return (
    <MapContainer center={[15, 75]} zoom={4} minZoom={3.5} maxZoom={8} style={{ height: "100%", width: "100%" }}>
      {/* Ocean basemap without borders */}
      <TileLayer
        url="https://server.arcgisonline.com/ArcGIS/rest/services/Ocean/World_Ocean_Base/MapServer/tile/{z}/{y}/{x}"
      />

      {/* Add ocean names */}
      {oceanLabels.features.map((feature, idx) => {
